...
#define qmi8658_log(format, ...) ESP_LOGI("imu", format, ##__VA_ARGS__)
...
extern void qmi8658_read_xyz(float *acc, float *gyro);
extern void qmi8658_read_sensor_data(float *acc, float *gyro);
...
extern qmi8658_euler_angles qmi8658_get_euler_angles(float gx, float gy, float gz, float ax, float ay, float az);
