...
unsigned char qmi8658_write_reg(unsigned char reg, unsigned char value)
{
	...
	buf[0] = reg;
	buf[1] = value;
	return ik_iic.transfer( buf, sizeof(buf), NULL, 0 );
}
...
unsigned char qmi8658_write_regs(unsigned char reg, unsigned char *value, unsigned char len)
{
	...
	return ik_iic.transfer( buf, len+1, NULL, 0 );
}
...
unsigned char qmi8658_read_reg(unsigned char reg, unsigned char* buf, unsigned short len)
{
	uint8_t tx_buf[10] = {0};
	tx_buf[0] = reg;
	return ik_iic.transfer( tx_buf, 1, buf, len );
}

void qmi8658_delay(unsigned int ms)
{
	uint64_t start_tick = 0;
	start_tick = ik_tim_get_tick_ms();
	while( ( ik_tim_get_tick_ms() - start_tick ) < ms );
	return;
}
...
{
	uint64_t start_tick = 0;
	start_tick = ik_tim_get_tick_us();
	while( ( ik_tim_get_tick_us() - start_tick ) < us );
	return;
}

void qmi8658_axis_convert(float *data_a, float *data_g, int layout)
{
	float raw[3],raw_g[3];
...
#if defined(QMI8658_USE_CALI)
void qmi8658_data_cali(unsigned char sensor, float *data)
{
	float data_diff[3];
...
#else
	// rad/s
	gyro[0] = (float)(raw_gyro_xyz[0]*_M_PI)/(g_imu.ssvt_g*180);		// *pi/180
	gyro[1] = (float)(raw_gyro_xyz[1]*_M_PI)/(g_imu.ssvt_g*180);
	gyro[2] = (float)(raw_gyro_xyz[2]*_M_PI)/(g_imu.ssvt_g*180);
#endif
}
